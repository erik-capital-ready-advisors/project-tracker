# Resolved spec — run 9b85cd

**Base spec:** `spec/spec-approved.md` (v1, approved 2026-08-17 by Erik; byte-identical to `spec-v1.md`)
**Spec approved:** YES — client-approved base, not a `spec-vN` draft.
**CRs folded, in parsed-integer order:** CR-001, CR-002, CR-003, CR-004, CR-005, CR-007
**CRs found and DELIBERATELY EXCLUDED:** CR-006

## What each CR changed

- **CR-001** (APPROVED 2026-08-18) — defects, regressions and releases. Adds `defect`, `release`,
  `release_requirement` entities and provisioning-identifier columns on `engagement`, taking the
  classified entity set to **21**. Adds FR-62 through FR-79; amends FR-12, FR-19, FR-30, FR-43,
  FR-45, FR-47, FR-50, FR-52, FR-58. Grows §4.3's deferred list by four entries without promoting
  any of it. Built as M1.x/M2.x — all Complete per `spec/prod.md`.
- **CR-002** (APPROVED 2026-08-19) — deletion versus append-only. Settles the conflict between two
  approved requirements. Rewrites FR-61: archiving is the reversible default, hard deletion is a
  separate, audited administrative action. Built as M1.10, Complete.
- **CR-003** (APPROVED 2026-08-20, Q9/Q10/Q11 answered) — detail and navigation. Adds FR-80 through
  FR-86, amends FR-55. Built as M2.7, Complete.
- **CR-004** (APPROVED 2026-08-20) — engagement reference scope. Resolves a conflict between FR-80,
  FR-81 and FR-83; **FR-81 wins**. Also touches FR-61. Sequencing/scope clarification, not new
  scope.
- **CR-005** (§3.2 APPROVED 2026-08-23; §3.1 and §3.3 APPROVED 2026-08-24, Q12–Q20 all resolved) —
  planned work and run history. Adds FR-87 through FR-96 (plus FR-96a/b/c); amends FR-14, FR-44,
  FR-58. Built as M2.8 (FR-92–FR-95, Complete) and M2.9 (FR-87–FR-91 + FR-96/a/b/c, Complete).
- **CR-007** (APPROVED 2026-08-24 by Erik; §3 approved and Q25, Q26, Q27 all RULED) — the
  fleet-coverage register. **Adds FR-104 through FR-109.** Amends §4.3 by promoting **bullet 2**
  ("Fleet-coverage register with a live trigger rule for when a stack has earned a specialist") out
  of Deferred and into buildable scope as milestone **M2.2**. Amends §10 **Q6**, which is now
  answered. Depends on **B61, which is closed**. **This is the entire buildable scope of run
  9b85cd.**

## CR-006 — excluded, deliberately, and this is not an omission

`spec/change-requests/CR-006-writeback-and-the-shape-of-phase-2.md` exists and is **NOT APPROVED**.
Its own status line reads: *"Q21, Q22 and Q23 RULED by Erik 2026-08-24 … **§3 itself is still NOT
APPROVED**, and **Q24 is NOT ruled** … Nothing here is built until §3 is approved."*

Therefore **FR-97 through FR-103 are NOT in force**, M2.6 Writeback is **not dispatchable**, and no
work-unit in this run may reference them. Three rulings on its open questions are not an approval of
its requirements. Folding it in would be the fleet authoring the requirements it then builds
against — the same act as an agent completing §7a or recording a §5a approval.

## Milestones already Complete in `spec/prod.md` — NOT decomposed by this run

M1.0, M1.1, M1.2, M1.3–M1.10 (Phase 1 MVP, all Done/Complete), **M2.7** (Complete 2026-08-24),
**M2.8** (Complete 2026-08-24), **M2.9** (Complete 2026-08-24, PR #5 merged and serving).

## Milestones NOT dispatchable — zero functional requirements between them

**M2.1, M2.3, M2.4, M2.5, M2.6.** Measured by fleet run `9d4658` on 2026-08-24 and recorded in the
tracker: these five carry **zero FRs**. Five exist in the approved spec only as bullets under §4.3
"Deferred (Phase 2+)"; M2.6 exists only in §12 Assumption 2. The tracker's `Not Started` word is
what makes them read as dispatchable; the spec does not. **This run does not decompose them from
prose.**

## Blocker reconciliation

`spec/prod.md`'s Active blockers table carries one entry naming M2.2 in its *Blocks* column:

> **B7** — Which stacks seed the register, and the trigger for earning an agent | Erik | **M2.2 only** |
> Spec Q6. Proposed trigger: two engagements and eight of Erik's hours, or one engagement where the
> gap blocks a dated milestone.

**B7 is discharged by CR-007's approval.** Erik's §10 Q6 answer is adopted verbatim as FR-106, and
Q25/Q26/Q27 settle counting, naming and the second limb. No unresolved blocker gates any work-unit
in this run. B57 (the manual gate cannot tell a fresh observation from a copied one) is open and is
a **handling rule** on this run's guide work, not a gate on dispatch.

---

# BUILDABLE SCOPE OF RUN 9b85cd — M2.2, the fleet-coverage register

**CR-007 §3, FR-104 through FR-109, verbatim.**
...
## Erik's rulings — settled, not re-openable by any specialist

- **Q25 RULED** — count **every** `work_session` row, and **say on the screen that they are all
  Mode 2 capture**. The claim is written where a reader sees it, so the day something else begins
  writing sessions the sentence is **visibly** wrong rather than quietly wrong.
- **Q26 RULED** — the route is **`/stacks`** and the screen is titled **"Stacks"**. **The word
  `coverage` is NOT to be reused** — not as a filename, not as a screen title, not as a component
  name. `src/lib/ingest/coverage.ts` already means requirement and test coverage (FR-45–FR-51), and
  a second meaning for the product's most overloaded word is a defect in itself.
- **Q27 RULED** — **ship limb one, record limb two as visibly UNMET.** The trigger's second limb
  (*one engagement where the missing agent blocks a dated contract milestone*) needs a
  `stack`→`blocker` link the schema does not have. It is **not** built, **not** silently dropped,
  and **not** worked around with new schema: it is listed as an unmet clause **on the screen** and
  in `prod.md`, the way M2.8's three clauses were ratified as-built.

## What CR-007 measured about the existing schema — read before proposing a migration

| Column | State, measured not assumed |
|---|---|
| `stack.name` | populated on demand by `upsertStack` — **one row exists, never seeded** |
| `stack.first_seen_at`, `stack.last_seen_at` | written by the capture path |
| `stack.agent_covering` | **exists, and nothing in the product reads or writes it** |
| `work_session.stack_id`, `work_session.duration_minutes` | both present |

**No route or screen reads the `stack` table.** The only reads anywhere are a detail view fetching a
stack's name (`src/lib/server/detail/work-item.ts:165`) and the upsert in
`src/lib/server/sessions/record.ts`. **This is M2.8's shape exactly.** Expect a read layer and a
screen, **not a schema**. **Migrations: zero expected.** No new entity, so the 21-entity §7a PASS
holds and **no new §7a row is needed**. If a migration is genuinely required, say why explicitly
rather than adding one quietly.

## Cost, stated up front

- **One new page route takes served routes 31 → 32**, so `manual-gate.sh` FAILs until
  `docs/user-guide.md` covers `/stacks`. **The guide update is budgeted into this milestone** — the
  lesson M2.8 paid ~157k tokens for and M2.9 then budgeted correctly.
- **B57 applies:** the manual gate cannot distinguish a fresh observation from a copied one.
  **Re-open the guide's rows rather than copying them forward.** On run `d4000f`, `doc1` re-opened
  all 30 and **15 had changed**, two of them claims already shipped and false.
- **The named risk is that the register is honest and empty.** The ledger holds **two** sessions
  today, both `delivery-ledger`, one with a NULL stack. The register's first true answer is "no
  stack has earned anything, and here is how little I have seen" — and FR-108 exists so that reads
  as a measurement rather than as a bug. **FR-108 is the requirement most likely to be built as
  decoration. It is not.**

---

# BASE SPEC — `spec/spec-approved.md`, verbatim below this line
