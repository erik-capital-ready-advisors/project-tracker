# Resolved spec — fleet run 9a320b

**Provenance block. This file is a BUILD ARTIFACT written by `project-lead`. It is not
`spec/` and nothing here amends `spec/`.**

- **Base spec:** `spec/spec-approved.md` (approved 2026-08-17, byte-identical to `spec-v1.md`).
  **Client-approved: YES.**
- **Change requests applied, in parsed-integer order:**
  - **CR-001** — defects, regressions, releases, a sixth answer (Broken), provisioning identifiers
    on `engagement`. FR-63–FR-79. Entity count 18 -> 21. **APPROVED 2026-08-18.**
  - **CR-002** — deletion vs append-only. Append-only wins absolutely; deletion stops at the audit
    boundary. No entity-count change. **APPROVED 2026-08-19.**
  - **CR-003** — detail views and cross-entity navigation, FR-80–FR-86 plus FR-55. No new entity.
    **APPROVED 2026-08-20; BUILT as M2.7 by run `eb2490`.**
  - **CR-004** — engagement reference scope, FR-81 wins. No code, no schema, no entity.
    **APPROVED 2026-08-20.**
  - **CR-005 §3.2 ONLY** — fleet run history, **FR-92 to FR-95**, as milestone **M2.8**.
    **APPROVED 2026-08-23 by Erik.** §3.1 (FR-87–FR-91) and §3.3 (FR-96) are **DRAFT, NOT
    APPROVED**, Q12–Q15 unanswered, and are **out of scope for this run**. Q16 resolved at
    approval: `/runs` is cross-engagement, by construction from FR-92's own wording.
- **Sequencing effect only, no scope effect:** CR-005 §5 orders M2.8 before M2.9.
- **Design direction:** §5a below carries `Approved design: NOT YET APPROVED`. `spec/prod.md`
  records, as Erik's own recorded action on 2026-08-22, that he ran `pnpm dev`, signed in, walked
  the six answer screens and several detail views, and **approved the design as-is, no changes
  requested**. That is carried forward as recorded state. `project-lead` did not write, infer or
  complete the §5a line and it still reads NOT YET APPROVED in `spec/`.

## THIS RUN'S SCOPE — M2.8, FR-92 to FR-95, verbatim from CR-005 §3.2
...
**Explicitly NOT in scope this run:** CR-005 §3.1 (FR-87–FR-91, planned work, `createWorkItem`,
plan parser) and §3.3 (FR-96, engagement filter). No migration. No new table. No new entity. The
21-entity `security-gate.sh` PASS must be unaffected.
