<!-- RESOLVED SPEC — build artifact, generated by project-lead run 29b583. DO NOT EDIT spec/. -->
# Resolved spec — run 29b583

**Base spec:** `spec/spec-approved.md` (v1, client-approved 2026-08-17, byte-identical to `spec-v1.md`; §7c appended 2026-08-21, committed `8795957`)
**Change requests applied, in ascending numeric order (parsed integer, not lexical):**
...
| CR-003 Detail views and cross-entity navigation | Approved 2026-08-20, Q9/Q10/Q11 answered at approval | Added FR-80–FR-86; reclassified **FR-55** as PARTIALLY MET (Q11: "links" means a hyperlink); added milestone **M2.7**, built first in Phase 2. No entity change, no §7a classification change. Q9 chose to ship on the existing `service_role` read path and raise **B29** |
| CR-004 Engagement reference scope | **Approved 2026-08-20** | Resolves FR-80 vs FR-81. **FR-81's eight entity kinds are exhaustive and closed**; `engagement` is deliberately outside them and renders as a plain `next/link` to the pre-existing `/registry/[slug]`. **No code, no schema, no entity-count change** — M2.7 already shipped this reading. Records the cost: an engagement reference carries no FR-83 dangling treatment, which is *unreachable* rather than defended, and §3 names the four events that would end that |

**Highest-numbered approved CR wins on conflict:** CR-004.

## What this run builds — scope, taken from `spec/prod.md`'s resume board

M1.1–M1.10 are **code complete**; M2.7 is **code complete, not Complete**. This run does **not**
rebuild any of them. It builds the five items `prod.md` names as fleet-dispatchable, plus the
§7b manual:

- **B33** — hoist the four-state prose renderer, five copies with three incompatible verification contracts, into one component in `src/components/` with one attribute contract.
- **B36** — `/questions/[id]` ships unreachable; decide where an open question is linked from and wire it (FR-81 `open_question` is one of the eight kinds).
- **B32** — the money-formatter split: `formatAmount` (`$111.00`) vs `money` (`111.00 USD`), one click apart since M2.7.
- **B40** — no sign-out control exists anywhere in the signed-in app.
- **`docs/user-guide.md`** — §7b, **not waived**. M1.10 does not reach Complete until it exists.

**Explicitly NOT in this run's scope, by `prod.md`'s own ruling** (recorded here so no unit picks
them up): **B39** and **B38** (both edit gate or Playwright-config files — gate-file integrity
needs Erik's explicit per-instance authorisation); the **M1.10 purge destruction path** (destroys a
live engagement, Erik's hands only); and **B37's follow-up** moving authentication server-side
(changes the auth architecture, wants a CR first).

**Standing gate-file rule, binding on every unit in this run:** `tests/m27-gate.test.ts` and
`e2e/m27-navigation.spec.ts` must not be edited. Commit `6210318` is the only commit that has ever
touched either, verified twice by different agents, and that fact is the evidence M2.7's gate was
written red before the milestone was dispatched. Preserve it.
...
## CR-004 — engagement reference scope (APPROVED 2026-08-20, highest-numbered, wins on conflict)
